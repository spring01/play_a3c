...
cluster_list = ['localhost:222%d' % num for num in range(int(sys.argv[2]))]
cluster = tf.train.ClusterSpec({'local': cluster_list})

task_number = int(sys.argv[1])
port = '222%d' % task_number
...
with tf.Session('grpc://localhost:{}'.format(port)) as sess:
    ...
    while True:
        ...
        print '***** at worker %d start *****' % task_number
        print sess.run(weights)
        print '***** at worker %d end *****' % task_number
    print(result)
